...
/**
 * Resolve a Java installation for Gradle.
 * Prefers JAVA_HOME, then Android Studio's bundled JBR, then common JDK locations.
 * Sets process.env.JAVA_HOME so the Gradle wrapper picks it up automatically.
 */
function ensureJavaHome() {
  // If JAVA_HOME already points to a valid JDK, keep it
  if (process.env.JAVA_HOME && fs.existsSync(path.join(process.env.JAVA_HOME, "bin", os.platform() === "win32" ? "java.exe" : "java"))) {
    return process.env.JAVA_HOME;
  }

  // Common Java install locations to probe (Android Studio's JBR is the easiest fallback)
  const candidates =
    os.platform() === "win32"
      ? [
  ...
  // Walk candidates and locate a directory containing bin/java
  for (const base of candidates) {
    if (!fs.existsSync(base)) continue;
    const javaExe = os.platform() === "win32" ? "java.exe" : "java";
    // Direct match (e.g. Android Studio's jbr is a JDK root)
    if (fs.existsSync(path.join(base, "bin", javaExe))) {
      process.env.JAVA_HOME = base;
      return base;
    }
    // Otherwise pick the first child that looks like a JDK
  ...
  // No Java found — Gradle will fail, but let it surface the error itself
  return null;
}
...
function build() {
  ...
  // Make sure Gradle can find a Java runtime
 ...
 * Auto-builds the APK if it doesn't exist yet.
 */
function install() {
  // Build the APK first if it hasn't been compiled yet
